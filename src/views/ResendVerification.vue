    ...
    <div class="max-w-lg text-xl">
      ...
      <div class="mt-3">
        <label class="block text-sm mb-2" for="email"
          >E-Mail-Adresse <span class="text-bb-red pl-5" v-if="message['email']">{{ message["email"][0] }}</span></label
        >
        <input class="w-full bb-input-dark" type="email" v-model="email" name="email" id="email" required />
      </div>
      <LoadingButton class="bb-btn bb-btn-dark mt-4" :loading="isLoading" @click.prevent="send()">abschicken</LoadingButton>
